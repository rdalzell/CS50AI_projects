...
class MinesweeperAI():
    ...
    def _infer_new_sentence(self, s1, s2):

        print ("INFER", s1, s2)

        if s1.cells.issubset(s2.cells):
            new_set = s2.cells - s2.cells
            new_count = s2.count - s1.count
            return (Sentence(new_set, new_count), True)
        else:
            return (set(), False)
        

    def _return_neighbours(self, cell, count):
